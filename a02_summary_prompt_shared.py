...
_ROLE_AND_OUTPUT = (
    ...
    "【要約の定義】要約＝冗長な言い回し・繰り返し・挨拶の削除。"
    "具体情報の省略や抽象化の言い換えではない。"
    "後から読んで理解・判断できるよう、必要な固有名詞・数値・手順・枠組みは残す。\n"
...
# financial / detailed 共通の「残す情報」指針
_DETAIL_RETENTION = (
    "【情報の残し方（重要）】\n"
    "・簡潔さより網羅性を優先。文字起こしが長い場合は出力も長くてよい。\n"
    "・1テーマ1行の抽象化は避ける。各論点は箇条書きで具体に書く。\n"
    "・複数の書籍・商品・制度・テーマを扱う場合（総集編など）は、"
    "各トピックごとに小見出しで区切り、トピック間の内容を混ぜない。\n"
    "・文字起こしにあれば次を可能な限り残す：\n"
    "  - 書籍・商品・制度・施策の正式名称、著者・発行者・出版社\n"
    "  - 分析フレームワーク・計算式・ステップ・チェックリストの名称と要点\n"
    "  - 具体的事例（企業名・銘柄・失敗談・比喩の対象）\n"
    "  - 選定基準・閾値・リストの内訳（例: 56銘柄の内訳）\n"
    "  - 数値（金額・率・期間・上限・取得単価・利回り）\n"
    "  - 警告・罠・注意喚起（例: 利回りの罠、鬼ホールドへの注意）\n"
    "【出所・固有名詞の正確性】\n"
    "・各論点・数値・手法・事例は、文字起こしで紹介されている書籍/トピックの下にのみ記載する。"
    "Aの本で触れていない内容をBの本の項目に書かない（総集編での混同を厳禁）。\n"
    "・書名・著者名・出版社・銘柄名・企業名・フレームワーク名は、"
    "**文字起こしの表記を最優先**する。外部知識・検索結果・一般的な正式名称で置き換えない。\n"
    "  音声認識の誤記と思われる箇所も、文字起こしのまま書くか「動画内表記: ○○」と明記する。"
    "別の語に差し替えない（例: セーブホールディングス→京王電鉄、"
    "これ1冊でわかる→失敗しない）。\n"
    "・似た語への置換・誤変換（例: SWOT→SWAN、著者名の取り違え）はしない。\n"
    "・数字一覧・メリット/リスク・アクションでも「どの本/論点の話か」を併記し、出所の取り違えを避ける。\n"
    "【出力量の目安】\n"
    "・正確性を保ちつつ、文字起こし本文の**20%以上**の文字数を目安に詳細を維持する"
    "（長い文字起こしほど要約も長くてよい。過度な圧縮で具体情報を落とさない）。\n"
)
...
_FINANCIAL_BODY = (
    ...
    + "\n"
    "【必須セクション（見出し付き）】\n"
    "1. 一言サマリー（誰向け・テーマ・動画の構成。複数本紹介なら冊数と各本の役割も）\n"
    "2. 対象者・前提条件（年齢、所得、世帯、雇用形態 等。動画内の言及を具体的に）\n"
    "3. 制度・商品・施策名と要点（トピック/書籍ごとに小見出し。"
    "各項目で名称・著者・独自の主張・手法・フレームワークを欠落させない）\n"
    "4. 数字一覧（金額・率・上限・期限・取得単価・利回り・銘柄数など）"
    "※文字起こしにあるもののみ。出所（どの本/論点か）が分かるよう併記。無い数字は捏造しない\n"
    "5. メリット / デメリット / リスク（具体例つき。動画内の失敗談・罠・シナリオも含める）\n"
    "6. 視聴者が取れるアクション（手順・時期・チェック項目・スクリーニング手順など、"
    "動画で紹介された具体的手順を段階的に）\n"
    "7. 注意・免責（個別事情で変わる点、動画主張≠公式見解）\n"
    "8. 公式確認先（下記参照リストから該当のみ列挙。無ければ「該当官公庁サイトで要確認」）\n\n"
    "※上記8セクションは必ず「1.」「2.」…「8.」の番号付き見出しで出力する。\n\n"
    "【ルール】\n"
    "・YouTube の断定表現は「動画内では〜との説明」と出所を区別する\n"
    "・推測は「推測:」と明示\n"
    "・補助金・非課税・住民税は詐欺・自治体差の注意を1文入れる\n"
    "・文字起こしにない制度改定年・税率・上限を書かない\n"
    "・セクション3・4・6は特に詳しく。一般論だけで埋めない\n"
    "・複数書籍の総集編では、各本の紹介パートの境界を意識し、"
    "手法・実績・数値の帰属先をそのパート内の本に限定する\n\n"
    "【禁止】文字起こしにない事実の捏造、過度な一般化、"
    "複数トピックの混同・帰属先の取り違え、「確実に儲かる」等の誇張をそのまま断定。"
)
    ...
    *,
    ...
    if mode == "minutes":
        ...
        body = (
            ...
            + "\n"
            "【構成の目安】\n"
            "・冒頭に全体の要約を2〜4文で書く（複数トピックなら構成も明示）。\n"
            "・見出し付きで章立てし、トピックごとに重要ポイント・背景・結論・"
            "視聴者向けの注意点・取りうるアクションを整理する。\n"
            ...
            "【禁止】文字起こしにない事実の捏造、過度な一般化、複数トピックの混同。"
        )
    elif mode == "custom":
